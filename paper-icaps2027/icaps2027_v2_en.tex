...
closed-loop operation. \textbf{(1)~Encoding barrier:} the revision
...
revisions). \textbf{(2)~Execution barrier:} every revision rebuilds
...
were processed twice. \textbf{(3)~Interface barrier:} when two agents
...
\section{Invisible Barriers in the Execution Layer}
\label{sec:exec}
...
\paragraph{Barrier 2: duplicated transport (ghost re-processing).}
...
\paragraph{Barrier 3: endpoint collisions (fleet-freezing deadlocks).}
