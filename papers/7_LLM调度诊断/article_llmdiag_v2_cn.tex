...
\author{SkyResearch}
\date{2026年9月6日 \quad v2.2 草稿（三档模型主表 + hard 视图质检 + CF--INT 差距 + 反事实执行 + unseen 留出首档；agentic 与 frontier 待补）}
...
Ollama 本地端点，32B 经 vLLM（4$\times$RTX 3090, TP=4, enforce-eager）；
均关闭思考链。}\label{tab:main}
\end{table}

全量结果给出五个发现：（F1）Qwen3 家族全面低于规则基线；hard 档 JRA 随规模
\textbf{单调降至零}（4B 0.038 $\to$ 8B 0.019 $\to$ 32B 0.000），easy 档非单调
（0.212 $\to$ 0.038 $\to$ 0.154）且三档全部远低于规则的 0.596——诊断质量不随
模型规模改善；（F2）hard 档崩塌至接近零——无事件 abductive 推断的缺口在调度域
被受控证实（hard 视图三防线质检 52/52 通过, 塌陷非视图缺陷所致, 见
\texttt{qc\_hard\_report.json}）；
（F3）\textbf{忠实—正确分离}：三档规模的忠实度均高达 0.97--1.00 且与正确率无关
（8B hard 定位全零、32B JRA 0.077 仍句句可回查）——``引用成立$\neq$诊断成立''的
实测证据，忠实度必须独立成指标；（F4）\textbf{基率忽视}：4B 把 30/34 个无故障
基线诊断为饥饿/拥塞/瓶颈，8B 更甚（29/33 判为 blocking），32B 达到 34/34 全部误诊
——三档模型都没有``正常运行包络''概念，且随规模加剧；（F5）\textbf{词表锚定失败}
...
（2 例）。
干预药房约束三档模型均 100\% 合规（104 案例全部落在引擎可执行词表内）。
frontier 模型与 agentic 取证能否扭转 F1/F2 是本基准的开放问题。[LLM\_SCALE\_TABLE]

\textbf{范围与局限}：本节结论的范围是\emph{冻结参数、非推理型、单轮打包}
的 LLM（Qwen3 家族 4B/8B/32B）。三点须知：(i) 4B/8B 经 Ollama 端点、
