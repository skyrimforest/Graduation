...
\author{SkyResearch}
\date{2026年9月4日 \quad v2 草稿（v2 数据集 + 规则塌陷基线；LLM 全量评测进行中）}
...
（部分分 0.5 封顶）；LLM 是否能从统计签名推断出规则推不出的结论，
由 hard 档 LLM--规则差值给出——反事实推理文献预言 LLM 在 abduction 型
任务上系统性弱~\cite{counterbench2025,execcf2025}，本基准给出首个调度域实证。
[LLM\_FULL\_TABLE]
