...
\textbf{场景家族}：(i) 注入故障 36（机器 16/双故障 12/随机扰动 8，标签由构造为真）；
(ii) 基线 6；(iii) 涌现配方 8（greedy+极小车队的饥饿活锁、cpsat+K=6 的走廊拥塞——
纯 FJSP/纯 MAPF 中不存在的耦合失效，无事件、只能从统计签名推断）；
...
全量结果给出五个发现：（F1）4B 规模 LLM 全面低于规则基线
（JRA 0.212/0.038 vs 0.596/0.346）；（F2）hard 档崩塌至接近零
（JRA 0.038）——无事件 abductive 推断的缺口在调度域被受控证实；
（F3）\textbf{忠实—正确分离}：忠实度高达 0.97--0.99 与正确率无关，
每条引用都真实可回查但结论系统性错误——这正是忠实度必须独立成指标、
且``引用成立$\neq$诊断成立''的直接证据；（F4）\textbf{基率忽视}：
34 个无故障基线中 30 个被过度诊断（饥饿/拥塞/瓶颈），模型没有
``正常运行包络''概念，把正常等待统计读成病理；（F5）\textbf{词表锚定失败}：
30 个走廊拥塞案例中 loc\_type=``corridor'' 从未出现（模型只会输出
task\_pool/agv/machine），但 cause 判断对 16/30——知道病类、说不出病位。
干预药房约束 100\% 合规（104 案例全部落在引擎可执行词表内）。
以上均为 4B 规模结论；frontier 模型与 agentic 取证是否缩小 F1/F2 缺口
是本基准的开放问题。[LLM\_SCALE\_TABLE]
