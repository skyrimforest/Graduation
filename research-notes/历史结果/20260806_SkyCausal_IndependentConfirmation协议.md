...
状态：`PRE_FREEZE_IMPLEMENTATION_GATE`。本文尚未授权 capture、planning 或
heldout。只有 root-level stress 编排、测试、最终 clean-image regression 与全部
hash 填充完成后，本文和 machine-readable config 才能一起改为
...
```text
SkyEngine commit:
  a1471066e08ff3463c17e9a803534ec2bfffbfbc

SkyEngine image:
  sha256:4a44e3009307adabeae7c5de3b40755977869ca194fbaa7eff31836f32c4d1ec
...
- [ ] root infrastructure stress 正式 runner 与 analyzer
- [ ] 对应 unit/tamper tests
- [ ] clean commit 和新 immutable image
- [ ] no-source-mount full regression
- [ ] protocol/config/claim matrix 最终 SHA
- [ ] 状态改为 `FROZEN_FOR_EXECUTION`

完成这些项前，禁止执行 seed `300..349` 的 capture。冻结后不得根据 v2 中间结果
修订 Gate；统计失败或基础设施失败都不能成为重跑同一 cohort 的理由。
